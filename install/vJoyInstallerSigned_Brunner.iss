...
//[Messages]
//WelcomeLabel2=This will install [name/ver] on your computer.%n%nNote that the driver signing process is sponsered by Brunner Elektronik AG.
...
procedure OpenBrunnerPage(Sender: TObject);
var
  ErrorCode: Integer;
begin
  ShellExec('open', 'https://shop.brunner-simulation.ch/', '', '', SW_SHOWNORMAL, ewNoWait, ErrorCode);
end;
...
begin
  ...
  BtnImage.OnClick := @OpenBrunnerPage;
  Cursor := crHand;
...
begin
  WizardForm.InnerNoteBook.Visible := (CurPageID <> Page.ID);
  BtnImage.Visible := (CurPageID = Page.ID);
end;
